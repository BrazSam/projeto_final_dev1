...
namespace ProjetoFinalDev1.Domain.Test
{
    public class UnitTesteTarefaItem
    {
        [Fact]
        public void CriarTarefa_Valida_NaoDeveLancarExcecao()
        {
            ...
            TarefaItem tarefa = TarefaItem.Criar(1, "Título da Tarefa", "Descrição da Tarefa",
                ETarefaStatus.Pendente, tipos, DateTime.Now, DateTime.Now.AddDays(7));
        }
        [Fact]
        ...
        public void CriarTarefa_Valida_VerificarNormalizado()
        {
            ...
            TarefaItem tarefa = TarefaItem.Criar(1, " Título da Tarefa ", " Descrição da Tarefa ", ETarefaStatus.Pendente, ETarefaTipo.Casa, DateTime.Now, DateTime.Now.AddDays(7)); Assert.Equal("TÍTULO DA TAREFA", tarefa.Titulo); // validando normalização do título
        ...
        {
            var exception = Assert.Throws<Exception>(() => TarefaItem.Criar(1, "", "Descrição da Tarefa", ETarefaStatus.Pendente, ETarefaTipo.Casa, DateTime.Now, DateTime.Now.AddDays(7))); Assert.Equal("TITULO_OBRIGATORIO", exception.Message); // validando a mensagem de exceção
        }
    }
}
