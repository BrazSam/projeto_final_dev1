...
using ProjetoFinalDev1.Domain.Services;
...
namespace ProjetoFinalDev1.Domain.Entities
{
    public class TarefaItem : Entity
    {
        ...
        public static TarefaItem Criar(int id, string titulo, string descricao, ETarefaStatus status, ETarefaTipo tipo, DateTime dataCriacao, DateTime dataLimite)
        {
            ...
            if (NormalizadoService.TextoVazioOuNulo(titulo)) throw new Exception("TITULO_OBRIGATORIO");
            titulo = NormalizadoService.LimparEspacos(titulo);
            titulo = NormalizadoService.ParaMaiusculo(titulo);
            if (NormalizadoService.TextoVazioOuNulo(descricao)) throw new Exception("DESCRICAO_OBRIGATORIO");
            ...
            // Para enums marcados com [Flags] (ETarefaTipo) permitimos combinações via bitwise OR, mas rejeitamos valores que contenham bits não definidos no enum.
            long allFlags = 0;
